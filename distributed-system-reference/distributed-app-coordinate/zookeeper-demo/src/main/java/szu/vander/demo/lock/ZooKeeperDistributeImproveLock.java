 ...
 *                         如是，则获得锁；不是，则注册前一节点的watcher,阻塞等待
 * 	                释放锁：删除自己创建的临时顺序节点
 */
public class ZooKeeperDistributeImproveLock implements Lock {
	...
	public ZooKeeperDistributeImproveLock(String lockPath) {
		super();
	...
	private void waitForLock() {
		...
		IZkDataListener listener = new IZkDataListener() {
			...
			public void handleDataDeleted(String dataPath) throws Exception {
				System.out.println(String.format("-----监听到节点%s被删除", dataPath));
				cdl.countDown();
			}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < currency; i++) {
            threadPoolExecutor.execute(()-> {
                ...
                ZooKeeperDistributeImproveLock lock = new ZooKeeperDistributeImproveLock("/distLock");
